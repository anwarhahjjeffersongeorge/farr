...
for (let giveStartValue of [0, 1]) {
  ...
  const startValueLabel = (giveStartValue) ? 'with start value › ' : ''
  const getExpected = e => (giveStartValue) ? e.map(ee => s + ee) : e
  ...
  test(`${startValueLabel} calls functions in order`, async t => {
    ...
    const f =  new Farr(fs)
    const result= await ((giveStartValue) ?  f.all({s}) :  f.all())
    t.deepEqual(result, getExpected(expected), `${startValueLabel} should do calls in order, passing mutated value`)
  })
